...
def create_small_img(Model, size=500):
    ...
    img = Image.open(Model.big_image.path)

    if img.height > size or img.width > size:
        img.thumbnail((size, size))
        img.save(Model.get_path_to_small_photo() + os.path.basename(Model.big_image.path))
        Model.min_image = "{0}{1}".format(Model.SMALL_IMG_PATH, os.path.basename(Model.big_image.path))
    else:
        Model.min_image = Model.big_image
